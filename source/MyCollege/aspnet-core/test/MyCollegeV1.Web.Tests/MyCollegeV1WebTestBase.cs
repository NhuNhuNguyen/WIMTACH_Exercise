...
{
    public abstract class MyCollegeV1WebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static MyCollegeV1WebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(MyCollegeV1WebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<MyCollegeV1DbContext> action)
        {
            using (var context = IocManager.Resolve<MyCollegeV1DbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<MyCollegeV1DbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<MyCollegeV1DbContext, Task> action)
        {
            using (var context = IocManager.Resolve<MyCollegeV1DbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<MyCollegeV1DbContext, Task<T>> func)
        {
            T result;
